...
switch(action.type){
  ...
  case FETCH_POST: return {...state,post:action.payload};
    ...
    case DELETE: return {...state,posts:state.posts.filter(post=>post._id!==action.payload)};
    case LIKE: return {...state,posts:state.posts.map((post)=>post._id===action.payload._id?action.payload:post )}; 
    case UPDATE: 
